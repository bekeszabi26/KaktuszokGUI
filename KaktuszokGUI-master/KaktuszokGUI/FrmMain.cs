...
namespace KaktuszokGUI
{
    public partial class FrmMain : Form
    {
        ...
        private void BtnDelete_Click(object? sender, EventArgs e)
        {
            ...
            {
                var valasz = MessageBox.Show("Biztos szeretnéd törölni?", "Törlés?", MessageBoxButtons.YesNo);
        ...
        private void DgvCactus_CellClick(object? sender, DataGridViewCellEventArgs e)
        {
            ...
            string tudomanyosnev = "";
            while (resp.Read())
            {
                tudomanyosnev = resp.GetValue(0).ToString();
            }
            MessageBox.Show($"{tudomanyosnev}");
        }
